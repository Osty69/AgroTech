...
async function fetchRealPrices() {
    ...
    nextUpdateTime = new Date(Date.now() + 60 * 60 * 1000).toISOString();
...
async function initializePrices() {
    ...
    if (currentPrices.length === 0 || !lastUpdateTime || 
        (new Date() - new Date(lastUpdateTime)) > 60 * 60 * 1000) {
        ...
        nextUpdateTime = new Date(new Date(lastUpdateTime).getTime() + 60 * 60 * 1000).toISOString();
    }
}
